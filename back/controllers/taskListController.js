...
class TaskListController {
    ...
    static async add(req, res) {
        ...
        if (!taskList) return res.status(500).json({ error: 'no task list sent' });
        if (!taskList.name) return res.status(500).json({ error: 'no task list name' });
        ...
        try {
            ...
            if (taskListRows.rows && taskListRows.rows.length > 0) return res.status(500).json({ error: 'name already taken' });
